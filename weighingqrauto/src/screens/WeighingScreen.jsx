...
export default function WeighingScreen({route}) {
  ...
  // Live weight state
  const [liveWeight, setLiveWeight] = useState(null);       // number from Arduino
  const [arduinoStatus, setArduinoStatus] = useState('waiting'); // 'waiting' | 'live' | 'stale'
  ...
  // Poll live weight from Arduino via server
  ...
  useEffect(() => {
    pollWeight(); // immediate first call
  ...
  const handleCapture = () => {
    const w = liveWeight;
    const n = parseFloat(nominalInput);
    if (!w || !n) { return; }
  ...
  const DETAILS = [
    {icon: 'inventory', label: 'Product', value: product?.productName || '—'},
    {icon: 'precision-manufacturing', label: 'Machine', value: machineError ? 'No machines' : (selectedMachine?.machineId || '—')},
    {icon: 'straighten', label: 'Unit', value: 'kg'},
  ];

  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.root} keyboardShouldPersistTaps="handled">
      {/* Spinning ring with live weight */}
      ...
      <View style={styles.detailsCard}>
        {DETAILS.map((d, i) => (
          <View key={d.label} style={[styles.detailRow, i < DETAILS.length - 1 && styles.detailBorder]}>
            <Icon name={d.icon} size={18} color={COLORS.muted} />
            <Text style={styles.detailLabel}>{d.label}</Text>
            {d.label === 'Machine' ? (
              <Pressable onPress={() => setShowMachinePicker(true)}>
      ...
      {arduinoStatus !== 'live' && (
        <Text style={styles.waitingHint}>
          ...
          Make sure Arduino is powered on and on the same network.
        </Text>
      )}
...
const styles = StyleSheet.create({
  ...
  spinRing: {position: 'absolute', width: RING_SIZE, height: RING_SIZE, borderRadius: RING_SIZE / 2, borderWidth: 4, borderColor: 'transparent', borderTopColor: COLORS.primary},
  ...
  detailsCard: {width: '100%', backgroundColor: COLORS.white, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.border, overflow: 'hidden', marginBottom: SPACING.lg, ...SHADOWS.card},
  ...
  detailLabel: {flex: 1, fontSize: 13, color: COLORS.muted, fontWeight: '500'},
  ...
  machineId: {fontSize: 14, fontWeight: '700', color: COLORS.primary, fontFamily: 'monospace'},
