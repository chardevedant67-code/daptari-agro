  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (recData.success) {
        ...
        setRecords([]); setStats({total: 0, passRate: 0});
      }
    } catch (_) {
      setRecords([]); setStats({total: 0, passRate: 0});
    }
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.scanRow}>
          <Pressable style={({pressed}) => [styles.scanCard, pressed && {opacity: 0.88}]} onPress={() => navigation.push('Scanner')}>
          ...
          <Pressable style={({pressed}) => [styles.scanCard, styles.scanCardAlt, pressed && {opacity: 0.88}]} onPress={() => navigation.push('WeighScan')}>
        ...
        {/* Start Session CTA */}
        <Pressable style={styles.sessionBtn} onPress={() => navigation.push('SessionStart')}>
          <Icon name="play-circle-filled" size={28} color="#fff" />
          <View style={{flex:1}}>
            <Text style={styles.sessionBtnTitle}>Start New Session</Text>
            <Text style={styles.sessionBtnSub}>Photo → Weight → QR → Save</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  sessionBtn:    {flexDirection:'row', alignItems:'center', gap:12, backgroundColor:COLORS.primary,
                  borderRadius:RADIUS.lg, padding:SPACING.md, marginBottom:SPACING.md,
                  shadowColor:'#1a227f', shadowOffset:{width:0,height:4}, shadowOpacity:0.3, shadowRadius:8, elevation:6},
  sessionBtnTitle:{fontSize:16, fontWeight:'800', color:'#fff'},
  sessionBtnSub:  {fontSize:11, color:'rgba(255,255,255,0.75)', marginTop:2},
});
