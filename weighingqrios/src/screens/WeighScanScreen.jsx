...
const BarcodeScanning = { scan: () => Promise.resolve([]) };
...
export default function WeighScanScreen() {
  ...
  useEffect(() => {
    if (step !== 3) return;
    setPolling(true); setManualMode(false); setAutoWeight(null); pollCount.current = 0;
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (data?.weight != null) {
          setAutoWeight(data.weight); setWeightVal(String(data.weight));
          setPolling(false); clearInterval(pollRef.current); anim.stop(); return;
        }
      } catch (_) {}
      if (pollCount.current >= 10) {
        setPolling(false); setManualMode(true); clearInterval(pollRef.current); anim.stop();
      }
    };
  ...
  const handleTakePhoto = async () => {
    ...
    } catch (_) { setError('Camera not available — use gallery instead'); }
  };
  ...
  const handlePickPhoto = async () => {
    ...
    try {
      ...
      if (result.didCancel || !result.assets?.length) return;
      setPhotoUri(result.assets[0].uri);
    } catch (_) { setError('Could not open gallery'); }
  };
  ...
  const handleSave = async () => {
    ...
    } catch (_) { addRecord({actualWeight: w, product}); }
          ...
          <React.Fragment key={label}>
            <View style={styles.stepItem}>
              <View style={[styles.stepCircle, active && styles.stepActive, done && styles.stepDone]}>
                {done ? <Icon name="check" size={12} color="#fff" /> : <Text style={[styles.stepNum, (active || done) && {color: '#fff'}]}>{num}</Text>}
              </View>
    ...
    return (
      <View style={{flex: 1, backgroundColor: '#0f172a'}}>
        <Camera 
          ...
          scanBarcode={Platform.OS === 'android'} 
        ...
        <View style={styles.scanFrameWrap}>
          <View style={styles.scanFrame}>
            {[styles.cTL, styles.cTR, styles.cBL, styles.cBR].map((cs, i) => <View key={i} style={[styles.corner, cs]} />)}
          </View>
          <Text style={styles.scanHint}>Align product QR within the frame</Text>

          {__DEV__ && (
            <Pressable 
              style={{marginTop: 15, padding: 8, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 6}}
              onPress={() => handleQRScanned({nativeEvent: {codeStringValue: 'PRD001'}})}
            >
              <Text style={{color: '#6366f1', fontSize: 11, fontWeight: 'bold'}}>SIMULATOR: MOCK SCAN PRD001</Text>
            </Pressable>
          )}
      ...
      <View style={styles.root}>
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          <View style={styles.productCard}>
            <View style={styles.productIconWrap}><Icon name="check-circle" size={22} color={COLORS.success} /></View>
            <View style={{flex: 1}}>
              <Text style={styles.productName}>{product?.productName}</Text>
              <Text style={styles.productMeta}>Batch: {product?.batchNumber}  ·  {product?.seedType}</Text>
          ...
          {photoUri ? (
            <View>
              ...
              <View style={styles.retakeRow}>
                <Pressable style={styles.retakeBtn} onPress={handleTakePhoto}><Icon name="refresh" size={16} color={COLORS.primary} /><Text style={styles.retakeTxt}>Retake</Text></Pressable>
                <Pressable style={styles.retakeBtn} onPress={handlePickPhoto}><Icon name="photo-library" size={16} color={COLORS.primary} /><Text style={styles.retakeTxt}>Gallery</Text></Pressable>
              </View>
            </View>
          ) : (
            <View style={styles.photoBox}>
              <Pressable style={styles.photoActionBtn} onPress={handleTakePhoto}><Icon name="add-a-photo" size={28} color={COLORS.primary} /><Text style={styles.photoBoxTxt}>Camera</Text></Pressable>
              <View style={styles.photoDivider} />
              <Pressable style={styles.photoActionBtn} onPress={handlePickPhoto}><Icon name="photo-library" size={28} color={COLORS.primary} /><Text style={styles.photoBoxTxt}>Gallery</Text></Pressable>
            </View>
          )}

          {!!error && <Text style={styles.errTxt}>{error}</Text>}

          <Pressable style={[styles.nextBtn, !photoUri && styles.nextBtnDisabled]} disabled={!photoUri} onPress={() => setStep(3)}>
  ...
  if (step === 3) {
    return (
      <View style={styles.root}>
        <View style={styles.header}>
          <Pressable style={styles.backBtn} onPress={() => setStep(2)}><Icon name="arrow-back" size={20} color={COLORS.text} /></Pressable>
          <Text style={styles.headerTitle}>Weigh & Record</Text>
          <View style={{width: 40}} />
        </View>
        {stepBar}
        <ScrollView contentContainerStyle={styles.scroll}>
          <View style={styles.productCard}>
            <View style={styles.productIconWrap}><Icon name="inventory" size={20} color={COLORS.primary} /></View>
            <View style={{flex: 1}}>
              <Text style={styles.productName}>{product?.productName}</Text>
              <Text style={styles.productMeta}>ID: {product?.productId}</Text>
            </View>
            {photoUri && <Image source={{uri: photoUri}} style={styles.thumbSmall} resizeMode="cover" />}
          </View>
          ...
          {autoWeight != null && !polling && (
            <View style={styles.autoWeightCard}>
              ...
              <Text style={styles.autoWeightTxt}>Auto captured: <Text style={{fontWeight: '800'}}>{autoWeight} kg</Text></Text>
            </View>
          )}

          {(manualMode || autoWeight != null) && (
            <View style={styles.inputCard}>
              <Text style={styles.inputLbl}>{manualMode ? 'Enter weight manually (kg)' : 'Actual Weight (edit if needed)'}</Text>
              <TextInput style={styles.weightInput} placeholder="e.g. 5.24" placeholderTextColor={COLORS.muted} keyboardType="numeric" value={weightVal} onChangeText={setWeightVal} />
              <Text style={[styles.inputLbl, {marginTop: 10}]}>Nominal / Expected Weight (kg)</Text>
              <TextInput style={styles.weightInput} placeholder="e.g. 5.00" placeholderTextColor={COLORS.muted} keyboardType="numeric" value={nominalVal} onChangeText={setNominalVal} />
            </View>
          )}

          {polling && <Text style={styles.pollNote}>If no reading in 20 sec, manual input will appear</Text>}
          {!!error && <Text style={styles.errTxt}>{error}</Text>}

          <Pressable style={[styles.nextBtn, (!weightVal || polling) && styles.nextBtnDisabled]} disabled={!weightVal || polling} onPress={handleSave}>
            {saving ? <ActivityIndicator color="#fff" /> : <><Text style={styles.nextBtnTxt}>Save Record</Text><Icon name="save" size={18} color="#fff" /></>}
          </Pressable>
        </ScrollView>
      </View>
    );
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={[styles.scroll, {alignItems: 'center'}]}>
        <View style={styles.doneCircle}><Icon name="check-circle" size={52} color={COLORS.success} /></View>
        <Text style={styles.doneTxt}>Record Saved!</Text>
        <Text style={styles.doneSub}>Photo and weight linked to product</Text>
        {photoUri && <Image source={{uri: photoUri}} style={styles.donePhoto} resizeMode="cover" />}
        ...
        <Pressable style={[styles.nextBtn, {width: '100%'}]} onPress={() => navigation.navigate('MainTabs')}>
...
function DoneRow({label, value, bold, mono, color}) {
  return (
    <View style={doneStyle.row}>
      <Text style={doneStyle.label}>{label}</Text>
      <Text style={[doneStyle.value, bold && {fontWeight: '800'}, color && {color}]}>{value || '—'}</Text>
    </View>
  );
}
...
const FRAME = 240; const CORNER_SZ = 32; const BW = 4;
...
const styles = StyleSheet.create({
  ...
  scanTopBar:  {position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: SPACING.lg, paddingTop: SPACING.xl, paddingBottom: SPACING.md},
