...
export default function WeighBeforeScreen({route}) {
  const navigation = useNavigation();
  const {sessionId, photoUrl} = route?.params || {};
  ...
  const captureWeight = async () => {
    const w = liveWeight ?? parseFloat(manualWeight);
    if (!w) { Alert.alert('No Weight', 'Place seed on scale or enter weight manually.'); return; }
    setCapturing(true);
    try {
      ...
      navigation.push('WeighAfter', {sessionId, photoUrl, beforeWeight: data.beforeWeight, beforeTime: data.beforeTime});
    } catch (e) {
  ...
  return (
    <WeighScreen
      step={2} title="Before Weight" stepLabel="Step 2 of 4"
      instruction="Place seed on scale before processing"
      liveWeight={liveWeight} iotStatus={iotStatus}
      capturing={capturing} onCapture={captureWeight}
...
export function WeighScreen({step, title, stepLabel, instruction, liveWeight, iotStatus, capturing, onCapture, onBack, activeStep}) {
  const navigation = useNavigation();
    ...
    <View style={s.root}>
      <View style={s.header}>
        ...
        <View>
          ...
          <Text style={s.headerSub}>{stepLabel}</Text>
        </View>
      </View>
      ...
      <View style={s.body}>
        <View style={s.progress}>
          {[1,2,3,4].map(n => (
            <View key={n} style={[s.progressDot, n <= activeStep + 1 && s.progressDotActive]} />
          ))}
        </View>
        ...
        <View style={s.weightCard}>
          <Text style={s.weightLabel}>{instruction}</Text>
        ...
        <Pressable style={[s.captureBtn, capturing && {opacity:0.7}]} onPress={onCapture} disabled={capturing}>
          {capturing
            ? <ActivityIndicator color="#fff" />
            : <><Icon name="done" size={22} color="#fff" /><Text style={s.captureTxt}>Capture {title}</Text></>
          }
        </Pressable>
      </View>
    </View>
...
const s = StyleSheet.create({
  ...
  weightValue:     {fontSize:52, fontWeight:'900', color:COLORS.primary, letterSpacing:-1},
  ...
  captureBtn:      {flexDirection:'row', alignItems:'center', justifyContent:'center', gap:8,
                    backgroundColor:'#10b981', borderRadius:RADIUS.md, paddingVertical:16, ...SHADOWS.md},
