  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setServerError(err.name === 'AbortError'
        ? `Request timed out (${baseUrl}) — check server is running`
        : `Server unreachable (${baseUrl}) — check WiFi and server`);
    }
    setLoading(false);
  };
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.heroWrap}>
        ...
        <Text style={styles.headingCaption}>Create your operator account to get started</Text>
      </View>
      ...
      {!success && (
        <View style={styles.form}>
          <PaperInput mode="outlined" label="Full Name" value={form.name}
            ...
            outlineStyle={{borderRadius: RADIUS.md}} error={!!errors.name} style={styles.input} />
          {!!errors.name && <Text style={styles.errText}>{errors.name}</Text>}

          <PaperInput mode="outlined" label="Email Address" value={form.email}
            onChangeText={t => {set('email')(t); setErrors(p => ({...p, email: ''}));}}
            keyboardType="email-address" autoCapitalize="none"
            left={<PaperInput.Icon icon="email-outline" />}
            outlineColor={COLORS.border} activeOutlineColor={COLORS.primary}
            outlineStyle={{borderRadius: RADIUS.md}} error={!!errors.email} style={styles.input} />
          {!!errors.email && <Text style={styles.errText}>{errors.email}</Text>}

          <PaperInput mode="outlined" label="Password" value={form.password}
            ...
            outlineColor={COLORS.border} activeOutlineColor={COLORS.primary}
            outlineStyle={{borderRadius: RADIUS.md}} error={!!errors.password} style={styles.input} />
          {!!errors.password && <Text style={styles.errText}>{errors.password}</Text>}

          <PaperInput mode="outlined" label="Confirm Password" value={form.confirm}
            onChangeText={t => {set('confirm')(t); setErrors(p => ({...p, confirm: ''}));}}
            secureTextEntry={!showConfirm}
            left={<PaperInput.Icon icon="lock-check-outline" />}
            right={<PaperInput.Icon icon={showConfirm ? 'eye-off-outline' : 'eye-outline'} onPress={() => setShowConfirm(p => !p)} />}
            outlineColor={COLORS.border} activeOutlineColor={COLORS.primary}
            outlineStyle={{borderRadius: RADIUS.md}} error={!!errors.confirm} style={styles.input} />
          ...
          <Button mode="contained" onPress={handleRegister} loading={loading} disabled={loading}
            icon="account-plus" contentStyle={styles.btnContent} labelStyle={styles.btnLabel}
...
const styles = StyleSheet.create({
  ...
  successBox:    {flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#dcfce7', borderRadius: RADIUS.md, padding: SPACING.md, marginBottom: SPACING.md},
