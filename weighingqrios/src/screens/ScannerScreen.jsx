...
import {Animated, Modal, Pressable, StyleSheet, Text, TextInput, View, Platform} from 'react-native';
...
const RNQRImageScanner = TurboModuleRegistry?.get?.('RNQRImageScanner') ?? NativeModules.RNQRImageScanner;
    ...
    <Animated.View style={[styles.overlay, {opacity: fade}]}>
      <Animated.View style={[styles.fetchCard, {transform: [{translateY: slideY}]}]}>
        <LinearGradient colors={['#0f172a', '#1e1b4b']} style={styles.fetchBanner}>
          <Animated.View style={[styles.fetchIconWrap, {transform: [{scale: pulse}], backgroundColor: current.color + '25', borderColor: current.color + '70'}]}>
        ...
        <View style={styles.fetchBody}>
          ...
          <View style={styles.progressTrack}>
            <View style={styles.progressSegments}>
              {STEPS.map((s, i) => (
                <View key={i} style={[styles.progressSeg, {backgroundColor: i <= step ? s.color : 'rgba(255,255,255,0.08)'}]} />
              ))}
            </View>
          </View>
...
function SimQrModal({visible, onCancel, onSubmit}) {
  const [text, setText] = useState('');
  return (
    <Modal visible={visible} transparent animationType="fade">
      <View style={styles.modalBackdrop}>
        <View style={styles.modalCard}>
          <Text style={styles.modalTitle}>Enter QR Code Value</Text>
          <Text style={styles.modalSub}>QR code not detected in image — enter the product code manually</Text>
          <TextInput
          ...
          <View style={styles.modalRow}>
            <Pressable style={styles.modalCancelBtn} onPress={() => { setText(''); onCancel(); }}>
            ...
            <Pressable
              ...
              disabled={!text.trim()}
            >
              <Text style={styles.modalSubmitTxt}>Scan</Text>
            </Pressable>
          </View>
        </View>
      </View>
...
export default function ScannerScreen() {
  ...
  const translateY = lineY.interpolate({inputRange: [0, 1], outputRange: [0, FRAME_SIZE - 3]});
  ...
  const handleGalleryPick = useCallback(async () => {
    ...
    try {
      ...
      const barcodes = await RNQRImageScanner.scanImage(result.assets[0].uri);
      if (!barcodes?.length) {
        setSimModalVisible(true);
        return;
      }
      await processQrValue(barcodes[0].value);
    } catch (err) {
      setLoading(false);
      scannedRef.current = false;
      setSimModalVisible(true);
    }
  }, [loading, processQrValue]);
  ...
  return (
    <View style={styles.root}>
      ...
      <SimQrModal
        ...
        onSubmit={(val) => { setSimModalVisible(false); processQrValue(val); }}
      />
      ...
      <View style={styles.frameWrap}>
        ...
        {/* Mock button for simulator */}
        {__DEV__ && (
          <Pressable 
            style={{marginTop: 20, padding: 10, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 8}}
            onPress={() => handleScannedCode({nativeEvent: {codeStringValue: 'PRD001'}})}
          >
            <Text style={{color: '#6366f1', fontSize: 12, fontWeight: 'bold'}}>SIMULATOR MOCK: SCAN PRD001</Text>
          </Pressable>
        )}
      ...
      <View style={styles.bottomBar}>
        <View style={styles.controls}>
          ...
          <Pressable style={[styles.sideBtn, torchOn && styles.sideBtnActive]} onPress={() => setTorchOn(t => !t)}>
...
const styles = StyleSheet.create({
  ...
  topBar:      {position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: SPACING.lg, paddingTop: SPACING.xl, paddingBottom: SPACING.md},
  circleBtn:   {width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center'},
  ...
  errorBox:    {flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: SPACING.md, backgroundColor: 'rgba(244,63,94,0.15)', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8},
  errorTxt:    {color: '#f43f5e', fontSize: 12, fontWeight: '600', flex: 1},
  bottomBar:   {paddingTop: SPACING.md, paddingBottom: 40, paddingHorizontal: SPACING.lg, backgroundColor: 'rgba(15,23,42,0.95)', alignItems: 'center'},
  ...
  overlay:     {...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(2,6,23,0.88)', alignItems: 'center', justifyContent: 'center', zIndex: 99},
  fetchCard:   {width: 310, backgroundColor: '#0f172a', borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.10)'},
  fetchBanner: {height: 150, alignItems: 'center', justifyContent: 'center'},
  fetchIconWrap:{width: 80, height: 80, borderRadius: 40, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5},
  ...
  modalBackdrop:   {flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24},
  modalCard:       {width: '100%', backgroundColor: '#1e293b', borderRadius: 16, padding: 24, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)'},
  modalTitle:      {fontSize: 17, fontWeight: '800', color: '#fff', marginBottom: 6},
  modalSub:        {fontSize: 12, color: '#94a3b8', marginBottom: 16, lineHeight: 18},
  modalInput:      {backgroundColor: '#0f172a', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, color: '#fff', fontSize: 15, borderWidth: 1, borderColor: 'rgba(99,102,241,0.4)', marginBottom: 16},
  modalRow:        {flexDirection: 'row', gap: 10},
  modalCancelBtn:  {flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.07)', alignItems: 'center'},
  modalCancelTxt:  {color: '#94a3b8', fontWeight: '600', fontSize: 14},
  modalSubmitBtn:  {flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: '#6366f1', alignItems: 'center'},
